 ...
 *   a unique hctx->output buffer per hctx; do not use reuse p->tmp_buf across
...
static int stream_deflate_compress(server *srv, connection *con, handler_ctx *hctx, unsigned char *start, off_t st_size) {
	...
	do {
		...
		if (z->avail_out == 0 || z->avail_in > 0) {
			...
			hctx->bytes_out += len;
			http_chunk_append_mem(srv, con, hctx->output->ptr, len);
			z->next_out = (unsigned char *)hctx->output->ptr;
			z->avail_out = hctx->output->size;
		}
	} while (z->avail_in > 0);
...
static int stream_bzip2_flush(server *srv, connection *con, handler_ctx *hctx, int end) {
	...
	do {
		...
		if (end) {
			 ...
			 * which would loop on BZ_FLUSH which BZ_FLUSH_OK
...
CONNECTION_FUNC(mod_deflate_handle_response_start) {
	...
	if (p->conf.max_compress_size /*(max_compress_size in KB; (len >> 10))*/
	    && (len >> 10) > (off_t)p->conf.max_compress_size) {
		return HANDLER_GO_ON;
	}
	...
	/*if (etaglen) { ... } *//* return if in file cache and updated con->write_queue */
